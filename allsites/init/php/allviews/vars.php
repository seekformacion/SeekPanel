<?php


$v['vars']['eqtempl'][0]="home";
$v['vars']['eqtempl'][1]="panel";
$v['vars']['eqtempl'][5]="cms";

$v['vars']['purl'][1]="http://cursodecursos.com";
$v['vars']['purl'][2]="http://masterenmasters.com";
$v['vars']['purl'][3]="http://fp-formacionprofesional.com";
$v['vars']['purl'][4]="http://oposicionesa.com";

$v['vars']['purlT'][1]="Cursos";
$v['vars']['purlT'][2]="Masters y postgrados";
$v['vars']['purlT'][3]="Formación profesional";
$v['vars']['purlT'][4]="Preparación oposiciones";



$v['vars']['eqtip'][1]['s']="Curso";                   			$v['vars']['tipPort'][1]=1;
$v['vars']['eqtip'][2]['s']="Master";							$v['vars']['tipPort'][2]=2;
$v['vars']['eqtip'][3]['s']="Postgrado";						$v['vars']['tipPort'][3]=2;
$v['vars']['eqtip'][4]['s']="Oposicion";						$v['vars']['tipPort'][4]=4;
$v['vars']['eqtip'][5]['s']="Formación Profesional";			$v['vars']['tipPort'][5]=3;
$v['vars']['eqtip'][6]['s']="Ciclo grado medio";				$v['vars']['tipPort'][6]=3;
$v['vars']['eqtip'][7]['s']="Ciclo Grado Superior";				$v['vars']['tipPort'][7]=3;
$v['vars']['eqtip'][8]['s']="Carrera universitaria";			$v['vars']['tipPort'][8]=2;
$v['vars']['eqtip'][9]['s']="Curso técnico";					$v['vars']['tipPort'][9]=1;
$v['vars']['eqtip'][10]['s']="Curso de doctorado";				$v['vars']['tipPort'][10]=2;
$v['vars']['eqtip'][11]['s']="Curso de Verano";					$v['vars']['tipPort'][11]=1;
$v['vars']['eqtip'][12]['s']="Diplomatura";						$v['vars']['tipPort'][12]=2;
$v['vars']['eqtip'][13]['s']="Doctorado";						$v['vars']['tipPort'][13]=2;
$v['vars']['eqtip'][14]['s']="Experto";							$v['vars']['tipPort'][14]=2;
$v['vars']['eqtip'][15]['s']="Feria";							$v['vars']['tipPort'][15]=1;
$v['vars']['eqtip'][16]['s']="Formación a empresas";			$v['vars']['tipPort'][16]=1;
$v['vars']['eqtip'][17]['s']="Formación Ocupacional";			$v['vars']['tipPort'][17]=1;
$v['vars']['eqtip'][18]['s']="Formación subvencionada";			$v['vars']['tipPort'][18]=1;
$v['vars']['eqtip'][19]['s']="Jornada/conferencias";			$v['vars']['tipPort'][19]=1;
$v['vars']['eqtip'][20]['s']="Licenciatura";					$v['vars']['tipPort'][20]=2;
$v['vars']['eqtip'][21]['s']="Manual";							$v['vars']['tipPort'][21]=1;
$v['vars']['eqtip'][22]['s']="Monografía";						$v['vars']['tipPort'][22]=1;
$v['vars']['eqtip'][23]['s']="Ocupacional";						$v['vars']['tipPort'][23]=1;
$v['vars']['eqtip'][24]['s']="Otros estudios";					$v['vars']['tipPort'][24]=1;
$v['vars']['eqtip'][25]['s']="Seminario";						$v['vars']['tipPort'][25]=1;
$v['vars']['eqtip'][26]['s']="Taller";							$v['vars']['tipPort'][26]=1;
$v['vars']['eqtip'][27]['s']="Tutorial";						$v['vars']['tipPort'][27]=1;
$v['vars']['eqtip'][28]['s']="White Paper";						$v['vars']['tipPort'][28]=1;
$v['vars']['eqtip'][29]['s']="Apunte";							$v['vars']['tipPort'][29]=1;

$v['vars']['eqtip'][1]['p']="Cursos";
$v['vars']['eqtip'][2]['p']="Masters";
$v['vars']['eqtip'][3]['p']="Postgrados";
$v['vars']['eqtip'][4]['p']="Oposiciones";
$v['vars']['eqtip'][5]['p']="Formación Profesional";
$v['vars']['eqtip'][6]['p']="Ciclos grado medio";
$v['vars']['eqtip'][7]['p']="Ciclos Grado Superior";
$v['vars']['eqtip'][8]['p']="Carreras universitarias";
$v['vars']['eqtip'][9]['p']="Cursos técnicos";
$v['vars']['eqtip'][10]['p']="Cursos de doctorado";
$v['vars']['eqtip'][11]['p']="Cursos de Verano";
$v['vars']['eqtip'][12]['p']="Diplomaturas";
$v['vars']['eqtip'][13]['p']="Doctorados";
$v['vars']['eqtip'][14]['p']="Expertos";
$v['vars']['eqtip'][15]['p']="Ferias";
$v['vars']['eqtip'][16]['p']="Formación a empresas";
$v['vars']['eqtip'][17]['p']="Formación Ocupacional";
$v['vars']['eqtip'][18]['p']="Formación subvencionada";
$v['vars']['eqtip'][19]['p']="Jornadas y conferencias";
$v['vars']['eqtip'][20]['p']="Licenciaturas";
$v['vars']['eqtip'][21]['p']="Manuales";
$v['vars']['eqtip'][22]['p']="Monografías";
$v['vars']['eqtip'][23]['p']="Ocupacional";
$v['vars']['eqtip'][24]['p']="Otros estudios";
$v['vars']['eqtip'][25]['p']="Seminarios";
$v['vars']['eqtip'][26]['p']="Talleres";
$v['vars']['eqtip'][27]['p']="Tutoriales";
$v['vars']['eqtip'][28]['p']="White Papers";
$v['vars']['eqtip'][29]['p']="Apuntes";



$v['vars']['eqmet'][1]['s']="Presencial";
$v['vars']['eqmet'][2]['s']="Semipresencial";
$v['vars']['eqmet'][3]['s']="Personalizado";

$v['vars']['eqmet'][4]['s']="A distancia";
$v['vars']['eqmet'][5]['s']="Online";
$v['vars']['eqmet'][6]['s']="En el extranjero";
$v['vars']['eqmet'][7]['s']="Otros";



$v['vars']['certi'][0]="No reglado.";
$v['vars']['certi'][1]="Certificado";
$v['vars']['certi'][2]="Homologado";
$v['vars']['certi'][3]="Autorizado";
$v['vars']['certi'][4]="Titulación Oficial";
$v['vars']['certi'][5]="Preparación Oficial";
$v['vars']['certi'][6]="Título Propio";
$v['vars']['certi'][7]="Homologable";
$v['vars']['certi'][8]="Convalidación";
$v['vars']['certi'][9]="Otros";
$v['vars']['certi'][10]="Acreditado";

$v['vars']['esmin'][0]="Estudios no requeridos";
$v['vars']['esmin'][1]="Estudios no requeridos";
$v['vars']['esmin'][10]="Certificado de escolaridad";
$v['vars']['esmin'][11]="Graduado escolar, EGB, 2ºBUP";
$v['vars']['esmin'][12]="ESO";
$v['vars']['esmin'][21]="FP I, Grado Medio";
$v['vars']['esmin'][23]="B.LOGSE, COU";
$v['vars']['esmin'][41]="FP II, Grado Superior";
$v['vars']['esmin'][51]="Diplomado";
$v['vars']['esmin'][61]="Licenciado";
$v['vars']['esmin'][71]="Doctorado";


$v['vars']['eqtip'][1]['c']="Curso";                   			
$v['vars']['eqtip'][2]['c']="Master";							
$v['vars']['eqtip'][3]['c']="Postgrado";						
$v['vars']['eqtip'][4]['c']="Oposicion";						
$v['vars']['eqtip'][5]['c']="F.P.";			
$v['vars']['eqtip'][6]['c']="Grado medio";				
$v['vars']['eqtip'][7]['c']="Grado Superior";				
$v['vars']['eqtip'][8]['c']="C. universitaria";			
$v['vars']['eqtip'][9]['c']="Curso técnico";					
$v['vars']['eqtip'][10]['c']="Doctorado";				
$v['vars']['eqtip'][11]['c']="Curso de Verano";					
$v['vars']['eqtip'][12]['c']="Diplomatura";						
$v['vars']['eqtip'][13]['c']="Doctorado";						
$v['vars']['eqtip'][14]['c']="Experto";							
$v['vars']['eqtip'][15]['c']="Feria";							
$v['vars']['eqtip'][16]['c']="F.A Empresas";			
$v['vars']['eqtip'][17]['c']="F.Ocupacional";			
$v['vars']['eqtip'][18]['c']="F.subvencionada";			
$v['vars']['eqtip'][19]['c']="Conferencias";			
$v['vars']['eqtip'][20]['c']="Licenciatura";					
$v['vars']['eqtip'][21]['c']="Manual";							
$v['vars']['eqtip'][22]['c']="Monografía";						
$v['vars']['eqtip'][23]['c']="Ocupacional";						
$v['vars']['eqtip'][24]['c']="Otros estudios";					
$v['vars']['eqtip'][25]['c']="Seminario";						
$v['vars']['eqtip'][26]['c']="Taller";							
$v['vars']['eqtip'][27]['c']="Tutorial";						
$v['vars']['eqtip'][28]['c']="White Paper";						
$v['vars']['eqtip'][29]['c']="Apunte";							





?>